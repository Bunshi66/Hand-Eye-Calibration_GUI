...
from API.source.models.constants import (
    JOINTS_COUNT,
    JOYSTICK_ACCEL_MAX_DEG_SEC,
    JOYSTICK_ACCEL_MAX_RAD_SEC,
    JOYSTICK_SPEED_MAX_DEG_SEC,
    JOYSTICK_SPEED_MAX_RAD_SEC,
)
...
class SimpleJoystickUI(Tk):
    def __init__(
        ...
        linear_set_jog_param_in_tcp: LinearMotion.set_jog_param_in_tcp,
        ...
        self.linear_set_jog_param_in_tcp = linear_set_jog_param_in_tcp
        ...
        for button, (
            ...
            self.binds.append(Binding(name, button.connect_release(self.stop)))
        ...
        self.binds.append(Binding("move", self.ui.move_btn.connect(self.move)))
        self.binds.append(
            Binding(
                "move",
        ...
        self.ui.free_drive_switch.bind("<ButtonRelease>", self.set_free_drive)
        self.ui.coordinate_system_combobox.bind(
            "<<ComboboxSelected>>",
            self.set_current_coordinate_system,
        )
        self.ui.orientation_units_combobox.bind(
            "<<ComboboxSelected>>",
            self.set_orientation_units_combobox_value,
        )
    ...
    def copy_rtd(self, type_: str):
        ...
        self.clipboard_append(
            str(self.act_j_pose())
            if type_ == MotionTypes.JOINT
            else str(self.act_l_pose(self.units))
        )
    ...
    def set_current_coordinate_system(self, event):
        ...
        if self.current_coordinate_system == GUICoordinateSystem.CTI:
            self.linear_set_jog_param_in_tcp("tcp")
        else:
            self.linear_set_jog_param_in_tcp("base")
    ...
    def set_speed_scale_value(self, event):
        ...
        self.scale_setup(velocity=scale_value, acceleration=scale_value)
    ...
    def paste_copied_data(self):
        clipboard = [
            data.strip() for data in self.clipboard_get().strip("[]").split(",")
        ]
    ...
    def update_rtd(self):
        ...
        for group in self.ui.rtd_groups:
            pose = (
                ...
                if group.type_ == MotionTypes.JOINT
                else act_l_pose
            )
