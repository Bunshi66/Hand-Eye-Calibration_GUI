...
from API.source.features.gui.bindings import (
    CoordinateEntryGroup,
    RTDGroup,
)
from API.source.features.gui.custom_button import (
    _CustomButton,
)
    ...
    def set_rtd(self):
        for i in range(0, 4, 2):
            rtd_group = RTDGroup(
                ...
                if i == 0
                else "Положение ЦТИ ( м )",
                type_=MotionTypes.JOINT if i == 0 else MotionTypes.LINEAR,
            )
            labels = []
            label = Label(
                self.rtd_frame,
