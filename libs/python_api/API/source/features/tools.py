...
def dataclass_to_tuple_recursive(
    class_object: Any,
) -> Tuple[Any, ...]:
    """
    Конвертация всех полей дата-класса в один общий кортеж.
    В процессе конвертации происходит распаковка всех списков, кортежей и
    вложенных датаклассов.

    Args:
        class_object: Объект дата-класса для распаковки.

    Returns:
        tuple: Значения полей дата-класса (списки, кортежи и вложенные
            датаклассы распакованы).
    """
    unpacked_values = ()
    ...
    for field in fields(class_object):
        ...
        if isinstance(value, (list, tuple)):
            unpacked_values += tuple(value)
        elif hasattr(value, "__dataclass_fields__"):  # if dataclass
            unpacked_values += dataclass_to_tuple_recursive(value)
        else:
            unpacked_values += (value,)
