...
class DigitalIO(object):
    ...
    def get_output(self, index: DigitalIndex) -> bool:
        ...
        _validate_index(index, range(AVAILABLE_DIG_OUT_INDEX_COUNT))
        byte_index = int(index / BITS_IN_BYTE)
        mask = 1 << (index % BITS_IN_BYTE)
    ...
    def set_output(self, index: DigitalIndex, value: bool) -> bool:
        ...
        dig_out_mask[int(index / BITS_IN_BYTE)] = 1 << (index % BITS_IN_BYTE)
        set_output_template.dig_out_mask = dig_out_mask
        dig_out = list(set_output_template.dig_out)
        dig_out[int(index / BITS_IN_BYTE)] = (1 if value else 0) << (
            index % BITS_IN_BYTE
        )
    ...
    def wait_input(
        ...
        _validate_index(index, range(AVAILABLE_DIG_IN_INDEX_COUNT))
        for _ in sleep(
            await_sec=await_sec,
            frequency=CHECK_FREQUENCY_SEC,
        ):
    ...
    def get_input_functions(
        self, index: Optional[DigitalIndex] = None
    ...
    def set_output_function(
        self, index: DigitalIndex, function: OutputFunction_
    ) -> bool:
