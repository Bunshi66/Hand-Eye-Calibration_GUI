...
from API.source.models.constants import (
    ...
    AVAILABLE_WRIST_DIG_IN_INDEX_COUNT,
    AVAILABLE_WRIST_DIG_OUT_INDEX_COUNT,
    BITS_IN_BYTE,
    CHECK_FREQUENCY_SEC,
...
class WristDigitalIO(object):
    ...
    def _get_input(self, index: int, inputs_range: Iterable) -> bool:
        ...
        """
        _validate_index(index, inputs_range)
    ...
    def get_output(self, index: DigitalWristIndex) -> bool:
        ...
        _validate_index(
            index,
            range(AVAILABLE_WRIST_DIG_OUT_INDEX_COUNT),
        )
        mask = 1 << (index % BITS_IN_BYTE)
    ...
    def wait_input(
        ...
        _validate_index(index, range(AVAILABLE_WRIST_DIG_IN_INDEX_COUNT))
        for _ in sleep(
            await_sec=await_sec,
            frequency=CHECK_FREQUENCY_SEC,
        ):
    ...
    def set_input_function(
        ...
        _validate_literal("in_f", function)
        _validate_index(index, range(AVAILABLE_WRIST_DIG_IN_INDEX_COUNT))
        return self._controller.send(
    ...
    def get_input_functions(
        self, index: Optional[DigitalWristIndex] = None
    ...
    def set_active_output(
        ...
        if self._rtd_receiver.get_data().wrist_mode == Wm.off:
            raise WristStateError(
                f"Can not set hold active function, when wrist mode is {Wm.off}"
            )
        _validate_index(
            wrist_index,
            range(AVAILABLE_WRIST_DIG_IN_INDEX_COUNT),
        )
        _validate_index(
            output_index,
            range(AVAILABLE_DIG_OUT_INDEX_COUNT),
        )
        _validate_literal("activation_type", activation_type)
    ...
    def set_output_function(
        ...
        _validate_literal("out_f", function)
        return self._controller.send(
