...
from API.source.ap_interface.controller_gravity import (
    ControllerGravity,
)
from API.source.ap_interface.controller_state import (
    ControllerState,
)
...
from API.source.ap_interface.safety_status import (
    SafetyStatus,
)
...
from API.source.models.classes.enum_classes.controller_commands import (
    ControllerUnlockCommand as Cun,
)
from API.source.models.classes.enum_classes.controller_commands import (
    Getters as Get,
)
...
class RobotApi(object):
    ...
    """Подкласс для настойки ориентации робота."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._controller: Controller = Controller(
            ip=self._ip, timeout=self._timeout, logger=self._logger
        )
        ...
        self._api_shutdown_was_called: bool = False

        self._init_subclasses()

        self._write_log("info", f"Client version: [{RobotInfo.client_version}]")
        if autoconnect:
    ...
    def connect(
        ...
        if read_only:
            self._write_log(
                "info", f"Connecting to Robot at [{self._ip}] in read only mode"
            )
    ...
    def _init_subclasses(self):
        ...
        self.controller_gravity = ControllerGravity(
            controller=self._controller,
            connection_state=self._connection_state,
        )
        self.wrist = Wrist(
    ...
    def _start_rtd_receiver(self) -> bool:
        ...
        if not self._rtd_receiver.connect():
            ...
            return False
        self._rtd_receiver.start_loop(error_callback=self._shutdown_from_thread)
        return True
    ...
    def _exit_program(
        ...
        """
        if self._api_shutdown_was_called:
            return
    ...
    def _shutdown(self, exc_info: ExcInfoType = None):
        """
        Основная логика отключения от Робота в главном потоке. При передаче
        ...
        if not self._api_shutdown_was_called:
            self._handle_exception(exc_info)
            self._api_shutdown_was_called = True
        self._shutdown_api_sockets()
        self._connection_state.disconnect()
        if (
            exc_info is not None
            and self._disconnection_callback is not None
    ...
    def _handle_exception(self, exc_info: ExcInfoType = None):
        """
        ...
        if exc_info:
            ...
            self._write_log("info", "API connection planned shutdown")

    def _shutdown_api_sockets(self):
        """
    ...
    def _get_server_core_version(self) -> str | None:
        ...
        response = self._controller.receive(Get.ctrlr_coms_get_sw_version, "3I")
        return (
